...
有时，**我们不希望类的某些成员函数在类外被调用，例如，在类外禁止调用类的拷贝构造函数**。在 C++98 标准中，通常的做法是显式声明类的拷贝构造函数，并将其声明为类的私有成员。而 C++11 标准提供了一种更简便的方法，**在函数的声明后面加上“=delete”，编译器就会禁止函数在类外调用，这样的函数称为已删除函数**。
...
template< typename U>
typedef SuckType<std::vector<int>, U, 1> NewTyp; // 不合法
```
