...
[[Mysql-如何正确的使用索引以及索引的原理 ](https://www.cnblogs.com/ManyQian/p/9076247.html)](https://www.cnblogs.com/ManyQian/p/9076247.html)
...
**聚簇索引和非聚簇索引：**聚簇索引和非聚簇索引表示的是一种数据存储的结构。和唯一索引、前缀索引、联合索引等有着概念上的区别。

**唯一索引、前缀索引、联合索引**等都叫做辅助索引，也可以称为非聚簇索引，因为通过这些索引只能在叶节点找到主键值和当前索引列的值。如果要想获取当前主键值对应行中的其他数据，还要通过主键进行一次**回表**来获取数据。
